...
export const ParallelExample: React.FC<React.HTMLAttributes<HTMLElement>> = ({
  ...
  return (
    <div className={merge("flex flex-col gap-4", className)} {...props}>
      {state === "tap1" && (
        <>
          <div className="flex flex-col gap-3">
            ...
            <p className="text-gray-600  ">
              Single-threaded means that only one task can be executed at a
              time. If a task is running, any other tasks will have to wait
              until the first. This can cause problems when a task is
              computationally expensive, like sorting a large array or rendering
              a large list.{" "}
            </p>
          </div>

          <div className="flex flex-col gap-4">
            <h3 className="font-bold text-purple-600 text-sm border-b border-gray-200 pb-2">
      ...
      <div className={isRunning ? "" : "hidden"}>
        <p className="text-gray-600  ">
          <strong className="text-sm text-purple-900">Running...</strong>
        </p>
      ...
      <div className="flex flex-col gap-4">
        {state === "tap1" && !isRunning && (
          <div className="flex flex-col gap-4">
            <p className="text-gray-600  ">
              ...
              <strong>{fibonacciIndexes[0]}</strong> in the main thread an
              review the time that it takes for calculating.
            </p>

            <p className="text-gray-600  ">
              This will block the main thread a couple of seconds, you'll also
              notice that the main menu will be unresponsive. This is because
              the main thread is gonna be busy. Please be patient and wait for
              the result.
            </p>
            ...
            <p className="text-gray-600  ">
              Once you are ready reviewin the code, click the button for start
              the calculation.
            </p>
        ...
        {state === "tap1" && isRunning && (
          <div className="flex flex-row gap-4 w-full">
            <p className="text-gray-600  ">
              ...
              thread. You could notice for example that the main is
              unresponsive, freeze or laggy.
            </p>
          </div>
        )}
            ...
            {!firstRunFinished && (
              <div className="flex flex-col gap-4">
                <p className="text-gray-600  ">
                  ...
                  progress bar wasn't even able to move cause the main thread
                  was busy.
                </p>

                <h3 className="font-bold text-purple-600 text-sm border-b border-gray-200">
                  But, what if you could run multiple tasks at the same time?
                </h3>

                <p className="text-gray-600  ">
                  With <strong className="text-sm ">easy-web-worker</strong> you
                  can create multiple workers instances from a single worker
                  file or template.
                </p>
            ...
            {firstRunFinished && (
              <>
                ...
                <form className="flex flex-col gap-4">
                  <label className="text-gray-600  flex flex-col gap-4">
                  ...
                  <div className="flex flex-col gap-4 border border-gray-500 rounded-sm p-3">
                    {new Array(messagesCount).fill(0).map((_, index) => (
                      <label
                        key={index}
                        className="text-gray-600 flex flex-col gap-4 animate-fade-in"
                      >
                  ...
                  <label className="text-gray-600 flex flex-col gap-4">
                    How many milliseconds do you want to wait before disposing
                    the workers?
                    <input
                      autoComplete="off"
                      className="w-24 border border-gray-500 rounded-sm indent-3"
        ...
        {state === "tap3" && !isRunning && (
          <>
            ...
            <p className="text-gray-600  ">
              The advantage of using a web worker is that you can execute
              multiple tasks in parallel, like for example calculating the
              fibonacci number 3 times.
            </p>

            <p className="text-gray-600  ">
              Normally performing the same calculation 3 times in a row would
              take 3 times the time that it takes for performing the calculation
              once, right?{" "}
            </p>
            ...
            <p className="text-gray-600  ">
              With <strong>EasyWebWorker</strong> you can create multiple
              workers instances from a single worker file or template. The{" "}
            ...
            <p className="text-gray-600  ">
              From the main thread perspective consider you worker as an WEB
              API, you can send multiple HTTP request to the same API, right?
              well, in the same way you can send multiple messages to the same
              EasyWebWorker, and if you are using concurrent workers, instead of
              queueing the messages, the EasyWebWorker will create a new worker
              on demand until the maxWorkers limit is reached.
            </p>
        ...
        {state === "tap4" && (
          <>
            {!firstRunFinished && (
              <>
                <p className="text-gray-600  ">
                  <strong>Finished Great!</strong> It took basically the same
                  time as the previous example, but now we compute the fibonacci
                  number 3 times in parallel!
                </p>
            ...
            {!firstRunFinished && (
              <>
                ...
                <p className="text-gray-600  ">
                  ...
                  reached. Once the limit is reached, the messages will be
                  queued; and once all the messages are processed, the workers
                  will be disposed to free up resources.
                </p>

                <p className="text-gray-600  ">
                  By default easy web worker is configured to use 1 worker at
                  the time, this worker is created once a new instance of easy
                  web worker is created, and it keeps alive unless the instance
                  is disposed programatically. When using concurrent workers
                  this are created on demand until the maxWorkers limit is
                  reached.
                </p>

                <p className="text-gray-600  ">
                  You can modify this and other configurations by passing a
                  second and optional parameter to the createEasyWebWorker
                  function. please take a look in the documentation
                  <a
      ...
      <p ref={timeTakenRef} className="text-gray-600  flex flex-col gap-4">
        <span>
